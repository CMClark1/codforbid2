...
group by a.CFV, a.VESSEL_NAME, b.TRIP, c.GEARCD_ID, b.LANDING_DATE, d.SPECSCD_ID, d.SET_NO, e.SETDATE",sep=" ")) |>
  ...
  test1 <- marfis.df |>
    ...
    dplyr::mutate(test=(cod/had)<0.8,
                  test=ifelse(test==TRUE | is.na(test), "not cod directed", "cod directed"))

  trips_cod_directed <- test1 |> dplyr::filter(test=="cod directed") |> dplyr::distinct(trip_id) |> dplyr::pull(trip_id)
  trips_not_cod_directed <- test1 |> dplyr::filter(test=="not cod directed") |> dplyr::distinct(trip_id) |> dplyr::pull(trip_id)

  trips_both <- test1 |> dplyr::distinct(trip_id) |> dplyr::filter(trip_id%in%trips_cod_directed & trip_id%in%trips_not_cod_directed) #test again in next step
  trips_onlycod <- test1 |> dplyr::distinct(trip_id) |> dplyr::filter(trip_id%in%trips_cod_directed & !trip_id%in%trips_not_cod_directed) #remove
  trips_notcod <- test1 |> dplyr::distinct(trip_id) |> dplyr::filter(!trip_id%in%trips_cod_directed & trip_id%in%trips_not_cod_directed) #clean data

  cod_directed1 <- test1 |> dplyr::filter(trip_id%in%trips_onlycod$trip_id) |>
